...
import { ArrowRight, Code, Cloud, Database, Cpu } from "lucide-react";
import { useScrollAnimation } from "@/hooks/useScrollAnimation";
import heroImg from "@/assets/service-hero-tcb.jpg";

const solutions = [
  { icon: Code, title: "Software Development Bootcamps", description: "Intensive coding programmes covering modern frameworks, agile methodologies, and DevOps practices for rapid technical capability building." },
  { icon: Cloud, title: "Cloud & Infrastructure Training", description: "Hands-on certification programmes for AWS, Azure, and GCP covering architecture, security, and cost optimization." },
  { icon: Database, title: "Data & Analytics Literacy", description: "Equip your workforce with data-driven decision-making skills, from basic analytics to advanced machine learning concepts." },
  { icon: Cpu, title: "AI & Automation Skills", description: "Practical training in AI tools, prompt engineering, RPA, and intelligent automation to boost productivity across every function." },
];
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative min-h-[50vh] flex items-center overflow-hidden">
        ...
        <div className="relative container mx-auto px-4 lg:px-8 py-28">
          ...
          <motion.p className="text-[18px] md:text-[20px] text-muted-foreground leading-[1.6] max-w-2xl" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
            Future-proof your workforce with cutting-edge digital skills. From cloud computing to AI, we build the technical capabilities your business demands.
          </motion.p>
        </div>
      </section>
      ...
      <section className="py-24 section-navy" ref={impactRef}>
        <div className="container mx-auto px-4 lg:px-8 grid lg:grid-cols-2 gap-16 items-center">
          <div>
            <motion.h2 className="text-[32px] md:text-[40px] font-bold mb-6" initial={{ opacity: 0, y: 20 }} animate={impactInView ? { opacity: 1, y: 0 } : {}}>Digital Skills, Real Results</motion.h2>
            <motion.p className="text-[16px] text-muted-foreground leading-[1.6]" initial={{ opacity: 0 }} animate={impactInView ? { opacity: 1 } : {}} transition={{ delay: 0.2 }}>
              Organizations that invest in digital upskilling see dramatic improvements in innovation velocity, operational efficiency, and talent retention.
            </motion.p>
          </div>
          <div className="grid grid-cols-2 gap-6">
            {[{ value: "78%", label: "Faster Digital Adoption" }, { value: "4x", label: "Innovation Output" }, { value: "55%", label: "Higher Retention" }, { value: "90%", label: "Skills Confidence" }].map((stat, i) => (
      ...
      <section className="py-16 bg-cap-blue text-center">
        <div className="container mx-auto px-4">
          <h2 className="text-[24px] font-bold mb-4 text-primary-foreground">Accelerate your digital transformation journey</h2>
          <a href="#" className="inline-flex items-center gap-2 bg-foreground text-background px-8 py-4 text-[16px] font-semibold hover:gap-3 transition-all">Explore programmes <ArrowRight className="w-5 h-5" /></a>
        </div>
      </section>
      <Footer />
    </div>
