...
const tcbLinks = [
  { label: "Leadership Development", path: "/what-we-do/services/tcb/leadership-development" },
  { label: "Digital & Technical Upskilling", path: "/what-we-do/services/tcb/digital-upskilling" },
  { label: "Team Effectiveness", path: "/what-we-do/services/tcb/team-effectiveness" },
  { label: "Innovation & Design Thinking", path: "/what-we-do/services/tcb/innovation-design-thinking" },
  { label: "Learning Experience Design", path: "/what-we-do/services/tcb/learning-experience-design" },
  { label: "Certification & Compliance", path: "/what-we-do/services/tcb/certification-compliance" },
];
      ...
      <div className="container mx-auto px-4 lg:px-8">
        <div className="hidden lg:flex items-center justify-between">
          <div className="flex items-center gap-0 overflow-x-auto scrollbar-hide">
            <span className="flex-shrink-0 pr-6 py-3 text-xs font-bold tracking-wider text-cap-orange uppercase whitespace-nowrap">TCB</span>
            ...
            {tcbLinks.map((item) => {
              ...
              return (
                <button key={item.path} onClick={() => navigate(item.path)} className={`relative flex-shrink-0 px-5 py-3.5 text-xs font-medium whitespace-nowrap transition-colors ${isActive ? "text-foreground" : "text-foreground/60 hover:text-foreground hover:bg-muted/50"}`}>
                  {item.label}
                  {isActive && <motion.div layoutId="tcb-underline" className="absolute bottom-0 left-2 right-2 h-[2px] bg-cap-orange" transition={{ type: "spring", bounce: 0.2, duration: 0.4 }} />}
                </button>
              );
            })}
          </div>
        ...
        <div className="lg:hidden">
          <button onClick={() => setMobileOpen(!mobileOpen)} className="w-full flex items-center justify-between py-3 text-sm font-medium text-foreground">
            <span className="flex items-center gap-2">
              <span className="text-xs font-bold tracking-wider text-cap-orange uppercase">TCB</span>
